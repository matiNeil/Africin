  ...
  const handlePay = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/payments/initiate", {
        ...
        body: JSON.stringify({ contentId: stream.id, method: "web", authToken: token }),
      });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
      <div className="relative w-full max-w-sm bg-[#0a0a0a] border border-white/8 rounded-3xl shadow-2xl overflow-hidden">
        ...
        <div className="relative h-36 overflow-hidden">
          <Image src={stream.thumbnail} alt={stream.title} fill className="object-cover object-top opacity-50" sizes="448px" />
          <div className="absolute inset-0 bg-gradient-to-t from-[#0a0a0a] via-[#0a0a0a]/60 to-transparent" />
          <div className="absolute bottom-4 left-5 right-5">
            <span className="bg-red-600 text-white text-[9px] font-bold px-2.5 py-0.5 rounded-full uppercase tracking-widest">
              {isConcert ? "Live Concert" : "Premiere"}
            </span>
            <h2 className="text-white font-bold text-lg leading-tight mt-1.5">{stream.title}</h2>
          </div>
        </div>
        ...
        <div className="px-5 pb-6 pt-1">
          ...
          <p className="text-zinc-500 text-xs leading-relaxed mb-5">
            Pay today — your live stream access unlocks on{" "}
