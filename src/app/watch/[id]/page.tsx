    ...
    <main className="min-h-screen bg-background pt-16">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 -mt-24 relative z-10 pb-16">
        <div className="grid lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2">
            ...
            {content.premiereDate && isUpcoming && (
              <div className="inline-flex items-center gap-3 bg-red-500/8 border border-red-500/20 rounded-2xl px-4 py-2.5 mb-5">
                ...
                <CountdownTimer targetDate={content.premiereDate} variant="theme" />
              </div>
            )}
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 mb-4 text-sm">
              ...
              <span className="text-subtle">·</span>
              <span className="text-muted">{content.duration}</span>
              {content.episodes && <><span className="text-subtle">·</span><span className="text-muted">{content.episodes} eps</span></>}
              <span className="border border-hairline-strong text-subtle text-[10px] px-2 py-0.5 rounded">{content.rating}</span>
            </div>

            <div className="flex flex-wrap gap-2 mb-5">
              {content.genre.map((g) => (
                <Link key={g} href={`/browse?genre=${g}`} className="bg-surface hover:bg-red-500/10 border border-hairline hover:border-red-500/30 text-muted hover:text-red-500 text-[10px] font-medium px-3 py-1.5 rounded-full uppercase tracking-wider transition-all duration-300">{g}</Link>
              ))}
            </div>
          ...
          {/* Related */}
          <div>
            <h2 className="text-subtle text-[10px] uppercase tracking-widest mb-4">More Like This</h2>
            <div className="space-y-3">
              {related.map((item) => (
                <Link key={item.id} href={`/watch/${item.id}`} className="group flex gap-3 items-center">
                  <div className="relative w-24 flex-none aspect-video rounded-xl overflow-hidden bg-surface border border-hairline">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-muted text-xs font-medium truncate group-hover:text-red-400 transition-colors">{item.title}</p>
                    <p className="text-subtle text-[10px] mt-0.5">{item.year} · {item.country}</p>
                  </div>
                </Link>
              ))}
              {related.length === 0 && (
                <p className="text-subtle text-xs">More titles coming soon.</p>
              )}
            </div>
          </div>
        </div>
