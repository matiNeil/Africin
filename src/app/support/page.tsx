  ...
  return (
    <main className="min-h-screen bg-background pt-24 pb-20">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Contact card */}
        <div className="rounded-3xl border border-red-500/15 bg-gradient-to-br from-red-950/8 dark:from-red-950/20 to-surface p-6 sm:p-8 mb-12">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-5">
            <div>
              <h2 className="font-display font-semibold text-xl text-foreground mb-1">Contact support</h2>
              <p className="text-muted text-sm">We usually reply within 1–2 business days.</p>
            </div>
            <a
              href={`mailto:${SUPPORT_EMAIL}`}
              className="inline-flex items-center gap-2.5 bg-red-500 hover:bg-red-600 text-white font-semibold text-sm px-6 py-3 rounded-full transition-all shadow-lg shadow-red-900/20 self-start"
            >
        ...
        {/* FAQ */}
        <div className="mb-14">
          <h2 className="font-display font-semibold text-2xl text-foreground mb-5">Frequently asked questions</h2>
          <div className="space-y-3">
            {FAQS.map(({ q, a }) => (
              <details
                key={q}
                className="group rounded-2xl border border-hairline bg-surface open:border-red-500/20 open:bg-red-950/10 transition-colors"
              >
                <summary className="flex items-center justify-between gap-4 cursor-pointer list-none px-5 py-4 text-foreground text-sm font-medium">
                  {q}
                  <svg
                    className="w-4 h-4 text-subtle flex-none transition-transform duration-300 group-open:rotate-180"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                ...
                <p className="px-5 pb-5 -mt-1 text-muted text-sm leading-relaxed">{a}</p>
              </details>
            ))}
          </div>
        </div>
        ...
        {/* Get the app */}
        <div className="rounded-3xl border border-hairline bg-surface p-6 sm:p-8 text-center">
          <h2 className="font-display font-semibold text-xl text-foreground mb-2">Get the Africin app</h2>
