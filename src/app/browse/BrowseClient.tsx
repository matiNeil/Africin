  ...
  return (
    <main className="min-h-screen bg-background pt-16 pb-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Header */}
        <div className="py-10">
          <h1 className="font-display font-semibold text-3xl sm:text-4xl text-foreground tracking-tight mb-1">Browse</h1>
          <div className="h-px w-12 bg-gradient-to-r from-red-500 to-transparent mt-2" />
          <p className="text-subtle text-xs uppercase tracking-widest mt-3">Explore African stories from across the continent</p>
        </div>

        {/* Search */}
        <div className="relative mb-6">
          <svg className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-subtle" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <input
            ...
            className="w-full bg-surface border border-hairline text-foreground placeholder-subtle rounded-2xl pl-11 pr-4 py-3 text-sm focus:outline-none focus:ring-1 focus:ring-red-500/50 transition-all"
          />
        </div>
        ...
        <div className="space-y-3 mb-8">
          {/* Type */}
          <div className="flex bg-surface rounded-full w-fit p-0.5 gap-0.5">
            {(["all", "movie", "series"] as const).map((type) => (
              <button key={type} onClick={() => setSelectedType(type)}
                className={`px-5 py-1.5 rounded-full text-[10px] font-semibold uppercase tracking-widest transition-all duration-300 ${
                  selectedType === type ? "bg-red-500 text-white" : "text-subtle hover:text-foreground"
                }`}>
              ...
              <button key={genre} onClick={() => setSelectedGenre(genre)}
                className={`px-3 py-1.5 rounded-full text-[10px] font-semibold uppercase tracking-wider transition-all duration-300 ${
                  selectedGenre === genre
                    ...
                    : "border border-hairline text-subtle hover:border-hairline-strong hover:text-muted"
                }`}>
                {genre}
              </button>
            ))}
        ...
        {filtered.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-32 text-center">
            ...
            <h3 className="font-display text-xl text-muted mb-2">Nothing found</h3>
            <p className="text-subtle text-sm mb-5">Try adjusting your filters</p>
